...
public class SendReceiveService extends Service {
	...
	private void initializeWorkQueue() {
		...
		workerThread = new WorkerThread(workQueue, "SendReceveService-WorkerThread");// the worker thread takes jobs from the work queue to execute
	...
	private void scheduleSecretRequiredIntent(int what, Intent intent) {
		...
		synchronized (workQueue) {// why is synchronized is used here? is it to
			...
			// somehow the works in the queue stalled, current workaround is to empty the queue every time
			/*workQueue.clear();
			workQueue.add(work);
			workQueue.notifyAll();*/
			if (hasSecret) {
	...
	private class SendReceiveWorkItem implements Runnable {
		...
		@Override
		public void run() {
			Log.w(TAG, "One sendreceive work is executing intent is "+intent+" and what is "+what);
			switch (what) {
			case RECEIVE_SMS:
			...
			case SEND_SMS:
				...
				return;
			case RECEIVE_MMS:
				mmsReceiver.process(masterSecret, intent);
				return;
			case SEND_MMS:
				mmsSender.process(masterSecret, intent);
				return;
			case DOWNLOAD_MMS:
				mmsDownloader.process(masterSecret, intent);
				return;
			}
		}
	}
