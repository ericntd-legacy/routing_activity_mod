...
public abstract class WirePrefix {
	// debugging
	private static final String TAG = "WirePrefix";
  ...
  private static String calculatePrefix(byte[] message, int byteCount) {
    try {
      ...
      String tmp = Base64.encodeBytes(runningDigest, 0, byteCount);
      Log.w(TAG, "the prefix is "+tmp+" of the message "+ message.toString());
      return tmp;
